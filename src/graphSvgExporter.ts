...
export function renderGraphSvg(graph: PositionedGraph, options: GraphSvgOptions = {}): string {
  ...
  const nodeSvg = graph.nodes.map((node) => renderNode(node, rules)).join("\n");
...
function renderNode(node: PositionedNode, rules: StyleRules): string {
  const rule = rules.nodes[node.type] ?? rules.nodes.base;
  const x = node.position.x - rule.width / 2;
  const y = node.position.y - rule.height / 2;
  const shape =
    rule.shape === "hexagon"
      ? `<polygon points="${hexagonPoints(node.position.x, node.position.y, rule.width, rule.height)}" fill="${rule.fill}" stroke="${rule.stroke}" stroke-width="1"/>`
      : `<rect x="${x}" y="${y}" width="${rule.width}" height="${rule.height}" rx="4" fill="${rule.fill}" stroke="${rule.stroke}" stroke-width="1"/>`;

  return [
    `<g id="${escapeXml(node.id)}">`,
    shape,
    `<text x="${node.position.x}" y="${node.position.y + 3}" font-family="${rules.label.fontFamily}" font-size="${rules.label.fontSize}" text-anchor="middle" fill="#233042">${escapeXml(node.displayName)}</text>`,
    `</g>`
  ].join("\n");
}
...
function calculateViewBox(graph: PositionedGraph): ViewBox {
  ...
  const xs = [...graph.nodes.map((node) => node.position.x), ...bendPoints.map((point) => point.x)];
  const ys = [...graph.nodes.map((node) => node.position.y), ...bendPoints.map((point) => point.y)];
